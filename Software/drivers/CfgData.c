...
const char **ppEnumName;
    ...
    *pStr = saveChar;		// restore character
	...
	case CFG_VAR_TYPE_TIME:	// 00:00 to 23:59
	    ...
	    AlarmSet (ALARM_SENSOR1_ON_TIME_1 + varIdx, hour, minute);
	    AlarmEnable (ALARM_SENSOR1_ON_TIME_1 + varIdx);
	    break;
...
char	*pStrBegin = *ppStr;
	...
	*(*ppStr++) = EOS;		// terminate string
	...
	if (! isspace((int)**ppStr)  &&  **ppStr != EOS)
...
ID_PARM	*pID;
	...
	{
	    case CFG_VAR_TYPE_TIME:		// 00:00 to 23:59
		if (! AlarmIsEnabled (ALARM_SENSOR1_ON_TIME_1 + i))
		{
		...
		{
		    AlarmGet (ALARM_SENSOR1_ON_TIME_1 + i, &hour, &minute);
		}
