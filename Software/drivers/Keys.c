...
typedef enum
{
    ...
    KEY_START_STOP,	 //!< the START_STOP Key is active
 ...
 *
 * There are 1 key and state supported:
 * - START_STOP (S2) collect data
 *
...
void	KeyInit (const KEY_INIT *pInitStruct)
{
    ...
    GPIO_PinModeSet (KEY_START_STOP_PORT, KEY_START_STOP_PIN, gpioModeInput, 0);
    GPIO_IntConfig  (KEY_START_STOP_PORT, KEY_START_STOP_PIN, false, false, false);
 ...
 *	number, e.g. @ref KEY_START_STOP_PIN.
 *
 * @param[in] extiLvl
...
KEYCODE	   keyCode;		// translated key code
	...
	case KEY_START_STOP_PIN: // START_STOP collecting data
            keyState = KEY_START_STOP;
	    keyCode  = KEYCODE_START_STOP_ASSERT;
    	    break;
